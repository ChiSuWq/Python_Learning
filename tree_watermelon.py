...
def chooseBestFeatureToSplit(dataSet):
	...
	bestInfoGain = 0.0; bestFeature = -1; bestValue = -1.0
	for i in range(numFeatures):
		if isinstance(dataSet[0][i], float):#如果不是连续值
            ...
            for splitValue in splitValues:
                bestLocalInfoGain, bestSplitValue = 0.0, -1
                newEntropy = 0.0
                for mode in ['lt','gt']:
...
def createTree(dataSet, labels):
    ...
    if bestFeatLabel in ['hantanglv', 'midu']:
